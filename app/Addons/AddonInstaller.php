...
final class AddonInstaller
{
    ...
    /**
     * На случай, если где-то на сервере всё же завалялся закэшированный
     * файл маршрутов (руками или старой версией deploy-скрипта):
     * `route:cache` для этого проекта в принципе не используется — сама
     * таблица маршрутов не статична, она зависит от того, какие
     * Module/Plugin сейчас активны в БД. Закэшировать её — значит
     * навсегда заморозить набор маршрутов на момент сборки: активировать
